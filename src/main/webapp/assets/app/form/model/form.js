...
class ExtraFormField {
    ...
    getControl() {
        if (!this.control) {
            this.control = new forms_1.FormControl(this.name, this.validators);
        }
        return this.control;
    }
    initValidators() {
        ...
        if (this.isTypeText() && this.minLength) {
            ...
            validators.push(forms_1.Validators.minLength(this.minLength));
        }
        if (this.isTypeText() && this.maxLength) {
            console.log('Adding maxLength validator', this.maxLength);
            validators.push(forms_1.Validators.maxLength(this.maxLength));
        }
        if (this.isTypeEmail()) {
        ...
        if (validators.length > 0) {
            this.validators = forms_1.Validators.compose(validators);
        }
    }
}
